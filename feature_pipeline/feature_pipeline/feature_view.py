...
def create(
    ...
    project = hopsworks.login(
        api_key_value=settings.SETTINGS["FS_API_KEY"], project="enrgy_consumption_yg987"
    )
    ...
    try:
        ...
        logger.info("No feature views found for energy_consuption_denmark_view.")
        feature_views = []

    for feature_view in feature_views:
        try:
            feature_view.delete_all_training_dataset()
        except hsfs.client.exceptions.RestAPIError:
            logger.error(
                f"Failed to delete training datasets for feature view {feature_view.name} with "
                f"version {feature_view.version}"
            )
    ...
    # Create feature view in the fiven feature group version
